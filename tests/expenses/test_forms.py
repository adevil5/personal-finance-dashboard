...
class TransactionFormTestCase(TestCase):
    ...
    def test_form_with_valid_receipt_upload(self):
        ...
        form = TransactionForm(
            data=self.valid_data, files={"receipt": receipt_file}, user=self.user
        )
        self.assertTrue(form.is_valid(), f"Form errors: {form.errors}")
    ...
    def test_form_save_creates_transaction(self):
        ...
        self.assertEqual(
            transaction.transaction_type, self.valid_data["transaction_type"]
        )
...
class TransactionFormWidgetTestCase(TestCase):
    ...
    def test_form_has_expected_widgets(self):
        ...
        # Check widget types
        self.assertEqual(
            form.fields["description"].widget.__class__.__name__, "TextInput"
        )
        ...
        self.assertEqual(
            form.fields["transaction_type"].widget.__class__.__name__, "Select"
        )
        ...
        self.assertEqual(
            form.fields["receipt"].widget.__class__.__name__, "ClearableFileInput"
        )
...
class TransactionFormIntegrationTestCase(TestCase):
    ...
    def test_transaction_create_view_htmx_valid(self):
        ...
        response = self.client.post(
            "/expenses/create/", data=self.valid_data, HTTP_HX_REQUEST="true"
        )
    ...
    def test_transaction_create_view_htmx_invalid(self):
        ...
        response = self.client.post(
            "/expenses/create/", data=invalid_data, HTTP_HX_REQUEST="true"
        )
    ...
    def test_file_upload_functionality(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with override_settings(MEDIA_ROOT=temp_dir):
                ...
                data = self.valid_data.copy()
                response = self.client.post(
                    "/expenses/create/", data=data, files={"receipt": receipt_file}
                )
                ...
                # Receipt field should be set (even if file validation
